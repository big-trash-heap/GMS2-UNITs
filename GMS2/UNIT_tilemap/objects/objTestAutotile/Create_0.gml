...
_obj.set = [UNIT_tileAuto16AtPix_set, UNIT_tileAuto16AtPix_set_cd];
_obj.reset = UNIT_tileAuto16AtPix_reset;
...
_obj = self.tileListObject[_ind++];
_obj.set = [UNIT_tileAuto47AtPix_set, UNIT_tileAuto47AtPix_set_cd];
_obj.reset = UNIT_tileAuto47AtPix_reset;
_obj.draw = function(_value, _x1, _y1, _x2, _y2) {
	// table bits:
	// ###############
	// ## 0 # 1 # 2 ##
	...
	// ###############
	if (_value > 0)
		_value = __UNIT_tileTable47()[? _value - 1];
	draw_set_alpha(1);
	draw_set_color(c_red);
	draw_line(_x1, _y1, _x2, _y1);
	draw_line(_x1, _y1, _x1, _y2);
...
_obj.addcall = function() {
	...
	if (keyboard_check_pressed(vk_space)) {
		...
		? UNIT_tileAuto47_region_cd
		: UNIT_tileAuto47_region)(self.tileCurrentTile,
			0, 0,
